...
const MyRestaurants = () => {
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:5001/business/restaurants/${user.id}`);
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:5001/business/add_new_restaurant", {
        method: "POST",
  ...
  const handleDelete = async (restaurantId) => {
    try {
      const response = await fetch(`http://127.0.0.1:5001/business/restaurants/${restaurantId}`, {
        method: "DELETE",
      });
      const result = await response.json();
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:5001/business/restaurants/${editingRestaurantId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
