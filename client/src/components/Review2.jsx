...
const Review2 = ({ restaurantId }) => {
    ...
    useEffect(() => {
        const fetchReviews = async () => {
            try {
                const response = await fetch(`http://rf-lb-1272531251.us-east-1.elb.amazonaws.com:5001/restaurants/${restaurantId}/reviews`);
                if (!response.ok) {
            ...
            try{
                const response = await fetch(`http://rf-lb-1272531251.us-east-1.elb.amazonaws.com:5001/restaurants/${restaurantId}/average_review`);
                if (!response.ok) {
                    throw new Error('Failed to fetch reviews');
                }
                const data = await response.json();
